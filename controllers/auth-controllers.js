...
const register = async (req, res) => {
  ...
  const verifyEmail = {
    ...
    html: `<a target="bllank" href="${BASE_URL}/api/auth/verify/${verificationCode}">Click verify email </a>`,
  };
...
const resendVerifyEmail = async (req, res) => {
  ...
  const verifyEmail = {
    to: email,
    subject: "Verify email",
    html: `<a target="bllank" href="${BASE_URL}/api/auth/verify/${user.verificationCode}">Click verify email </a>`,
  };
...
const login = async (req, res) => {
  ...
  if (!user.verify) {
    throw HttpError(401, "Emmail not verify");
  }
